...
1. `paths`를 이용 각 지점마다 갈 수 있는 지점과 걸리는 시간을 dict 구조로 생성한다.
   - 이때 `try`, `except`를 이용해 원소 추가
2. `intensity` 집합을 미리 생성한다. 각 1 ~ n 까지의 지점이 있기 때문에 n+1에 최대값 보다 1 더 큰 10000001값들을 곱한 리스트를 생성한다.
   - 이때 출발지인 입구에 해당하는 `gates` 지점들을 0으로 설정한다.
3. 입구인 `gates`에서 부터 시작, `hiking_trail`을 이용해 갈 수 있는 지점과 걸리는 시간을 이용해 현재 지점의 `intensity`값과 다음 지점까지의 시간 중 더 큰 값을 `max_time`에 저장. 다음 지점의 `intensity`값과 `max_time`을 비교해 작을 경우 이동한다.
   - 이동 시 꼭대기 summits에 해당하는 지점이 아닐 경우 trail에 저장해 다음 시작 지점들을 저장한다.
   - 검색속도를 높이기 위해 `summits`는 `set` 형태로 저장한다.
   - 최종적으로 `intensity`에 각 지점마다 최소 값들이 저장된다.
4. 최종 산봉우리와 최소 `intensity`를 출력하기 위해 `summits` 지점들 중 임의의 `summit`를 이용, `[summit, intensity[summit]]` 형태의 리스트 중 `intensity[summit]`이 가장 작은 값을 출력한다.
