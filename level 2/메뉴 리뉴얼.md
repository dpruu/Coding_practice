...
def solution(orders, course):
    ...
    for n in course:
        ...
        for menu in orders:
			...
			# 주문한 단푼메뉴 조합을 이용해 combination으로 만들 수 있는 조합을 생성
            for li in combinations(menu_list,n):
                m_set = ''.join(sorted(li))
                # course에 필요한 요리 개수 만큼 단품메뉴를 합쳐 new_set에 메뉴 추가
                # 각 코스가 나온 횟수 count
                if m_set in new_set:
                    new_set[m_set] += 1
                else:
    	...
    	# count가 가장 큰 코스 메뉴들을 answer에 추가
        answer += [set_menu for set_menu, cnt in new_set.items() if cnt > 1 and cnt == max(new_set.values())]

        
    return sorted(answer)
	# return 값으로 오름차순으로 정렬되야 하기 때문에 sorted 사용
```
...
def solution(orders, course):
    ...
    for course_size in course:
        ...
        # Counter의 most_common()은 데이터의 개수가 많은 순으로 정렬된 배열을 return
