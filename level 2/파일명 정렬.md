...
def solution(files):
    ...
    for i in range(len(files)):
        ...
        for t in files[i]:
            ...
            HEAD += t
            cnt += 1
            # HEAD에는 숫자가 아닌 문자만 있기 때문에 isdigit을 이용해 숫자일 경우 반복 종료
        for d in files[i][cnt:]:
            if not d.isdigit():
                break
            NUMBER += d
            cnt += 1
        	# NUMBER은 반대로 숫자가 아닐 경우 반복 종료 
        TAIL = files[i][cnt:]
        # HEAD, NUMBER을 제외한 나머지는 TAIL
        result.append([HEAD.lower(), int(NUMBER), TAIL, files[i]])
